...
* `provider` specifies which cloud provider to use for the deployment. For AWS, you will write `aws`. Possible options include `do` for Digital Ocean and `gcp` for Google Cloud Platform.
...
In this section, you need to specy the region and instance types, and number of nodes that you would like to use for deployment. To see available instance types and pricing, please refer to [AWS website](https://aws.amazon.com/ec2/instance-types/)
